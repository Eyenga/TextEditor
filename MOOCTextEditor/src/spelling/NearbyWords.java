...
public class NearbyWords implements SpellingSuggest
{
	...
	{
		// for each position in the s and for all possible characters
		for (int index = 0; index <= s.length(); index++)
		{
			for (char charCode = 'a'; charCode <= 'z'; charCode++)
			{
	...
	/**
	 * Returns a list of suggested words from the dictionary that closely match the
	 * spelling of the given (misspelled) word.
	 * 
	 * @param word
	 *            The misspelled word
	...
	public List<String> suggestions(String word, int numSuggestions)
	{
		...
		{
			// Remove word at the front of the queue and find all real words one edit distance from it
			List<String> suggestion = distanceOne(queue.remove(0), true);
			
			// Remove words that have already been visited
			suggestion.removeAll(visited);
			
			// Append remaining words to relevant list
			queue.addAll(suggestion); 
			visited.addAll(suggestion);
			retList.addAll(suggestion);	
		}
